...
<head>
    ...
    <title>ShopKu - Katalog Produk</title>
...
<body class="bg-slate-50">
    ...
    <script type="text/babel">
            ...
            return (
                <nav className="bg-white navbar-shadow sticky top-0 z-50">
                    <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
                        <a href="#" className="text-2xl font-bold text-slate-800" style={{fontFamily:'Playfair Display, serif'}}>
            ...
            return (
                <div className="bg-white rounded-2xl overflow-hidden shadow-sm card-hover border border-slate-100 flex flex-col">
                    <a href={`<?= base_url('detail') ?>/${product.id}`}>
                        <div className="h-52 flex items-center justify-center bg-slate-50 p-4 relative">
                    ...
                    <div className="p-4 flex flex-col flex-1">
                        ...
                        <h3 className="text-slate-800 font-semibold text-sm line-clamp-2 mb-2 leading-snug flex-1">
                            {product.title}
                        </h3>
                        {/* Star rating */}
            ...
            return (
                <aside className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-6 h-fit sticky top-24">
                    ...
                    <div className="flex items-center justify-between">
                        <h2 className="text-slate-800 font-bold text-lg" style={{fontFamily:'DM Sans, sans-serif'}}>Filter</h2>
                        ...
                        <div className="flex flex-col gap-2">
                            {["all", ...categories].map(cat => (
                                <button
                                    ...
                                    className={`filter-chip text-left text-sm px-3 py-2 rounded-lg border font-medium transition-all ${
                                        activeCategory === cat
            ...
            return (
                <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between mb-6">
                    ...
                    >
                        ...
                        />
                        {keyword && (
                            <button type="button" onClick={() => onKeyword('')} className="text-slate-400 hover:text-slate-600">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-1 bg-white border border-slate-200 rounded-lg p-1">
                            <button
                                onClick={() => onView('grid')}
                                className={`p-1.5 rounded ${view === 'grid' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-slate-600'}`}
                                title="Grid view"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <button
                                onClick={() => onView('list')}
                                className={`p-1.5 rounded ${view === 'list' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-slate-600'}`}
                                title="List view"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
        ...
        function EmptyState({ onReset }) {
            return (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <button
                        onClick={onReset}
                        className="btn-primary bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2 rounded-xl"
                    >
                        Reset Filter
                    </button>
                </div>
        ...
        function Toast({ message, visible }) {
            ...
            return (
                <div className="fixed bottom-6 right-6 bg-slate-800 text-white text-sm font-medium px-5 py-3 rounded-xl shadow-lg z-50 flex items-center gap-2 transition-all">
        ...
        function App() {
            ...
            /* --- Filter + Sort (useMemo agar efisien) --- */
            const filteredProducts = useMemo(() => {
                ...
                if (sortBy === 'popular')    result.sort((a, b) => (b.rating?.count || 0) - (a.rating?.count || 0));
                    ...
                    <main className="max-w-7xl mx-auto px-4 py-8">
                        <div className="flex flex-col lg:flex-row gap-8">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                {/* Error state */}
                                {error && (
                                    <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl px-5 py-4 text-sm mb-6 flex items-center gap-2">
                                        <span>⚠️</span> {error}
                                    </div>
                                )}
                                ...
                                {loading ? (
                                    <div className={viewMode === 'grid'
                                        ? "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-5"
                                        : "flex flex-col gap-4"}>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        {filteredProducts.map(product => (
                                            <ProductListCard key={product.id} product={product} onAddCart={handleAddCart} />
                                        ))}
                                    </div>
                                )}
                            </div>
